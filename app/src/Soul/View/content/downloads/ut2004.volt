{#

<div class="row">
    <div class="container">
        <h1>Unreal Tournament 2004 Screenshots + Installatie instructies</h1>
        <div class="col-md-12 color0 pb15 pt15">


        <h1>Installatie instructies</h1>
        <p>Download de file en installeer deze naar de plek naar keuze<br />
        In de Unreal Tournament 2004 map staat het bestand play-UT2004.exe waarmee het spel gespeeld kan worden</p>

        <H1>Screenshots : </H1>
        Klik op de plaatjes om ze in het groot te bekijken<br />
        {{ image('img/downloads/ut2004_ingame.jpg' ,'height': 250, 'width': 250)}}
        {{ image('img/downloads/ut2004_ingame2.jpg' ,'height': 250, 'width': 250)}}
        {{ image('img/downloads/ut2004_ingame3.jpg' ,'height': 250, 'width': 250)}}

        </div>
    </div>
</div>
#}
